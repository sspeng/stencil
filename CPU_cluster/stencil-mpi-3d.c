...
void create_dist_grid(dist_grid_info_t *grid_info, int stencil_type) {
    ...
    grid_info->local_size_z = grid_info->global_size_z / 2;
    ...
    for(int t = 0; t < nt; ++t) {
        ...
        if(pid % 2 == 0){ //yzplane
            MPI_Sendrecv((void *)(a0 + x_end - 1), 1, yzplane, pid + 1, pid, \
                         (void *)(a0 + x_end), 1, yzplane, pid + 1, pid + 1, MPI_COMM_WORLD, &status);
        } 
        else{
            MPI_Sendrecv((void *)(a0 + x_start), 1, yzplane, pid - 1, pid, \
                         (void *)(a0 + x_start - 1), 1, yzplane, pid - 1, pid - 1, MPI_COMM_WORLD, &status);
        }

        if((pid / 2) % 2 == 0){//xzplane
            MPI_Sendrecv((void *)(a0 + (y_end - 1) * ldx), 1, xzplane, pid + 2, pid, \
                         (void *)(a0 + y_end * ldx), 1, xzplane, pid + 2, pid + 2, MPI_COMM_WORLD, &status);
        }
        else{
            MPI_Sendrecv((void *)(a0 + y_start * ldx), 1, xzplane, pid - 2, pid, \
                         (void *)(a0 + (y_start - 1) * ldx), 1, xzplane, pid - 2, pid - 2, MPI_COMM_WORLD, &status);
        }

        if(pid / 4 == 0){ // xyplane
            MPI_Sendrecv((void *)(a0 + (z_end - 1) * ldx * ldy), 1, xyplane, pid + 4, pid, \
                         (void *)(a0 + z_end * ldx * ldy), 1, xyplane, pid + 4, pid + 4, MPI_COMM_WORLD, &status);
        }
        else{
            MPI_Sendrecv((void *)(a0 + z_start * ldx * ldy), 1, xyplane, pid - 4, pid, \
                         (void *)(a0 + (z_start - 1) * ldx * ldy), 1, xyplane, pid - 4, pid - 4, MPI_COMM_WORLD, &status);
        }
